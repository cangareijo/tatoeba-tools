...
source = set()
target = set()
for line in open(argv[3], encoding = "utf-8"):
  split = findall(r"[^\t\n]+", line)
  if split[1] == argv[1]:
...
print("Reading links...")
links = set()
for line in open(argv[4], encoding = "utf-8"):
  split = findall(r"[^\t\n]+", line)
...
for line in open(argv[3], encoding = "utf-8"):
  ...
  if int(split[0]) in links:
    ...
    for length in lengths:
      for i in range(len(split[2]) - length + 1):
        if split[2].lower()[i : i + length] in queries:
          words.add(split[2].lower()[i : i + length])
    for word in words:
      translations[word] += 1
